...
ChainSwitchScheduler : Object {
    classvar < version = "v0.2";  // or keep your current version string
    ...
    *new { |controller, verbose = true|
    ...
    init { |ctrl, v|
        ...
        verbose = v;
        if (verbose) {
            ("[ChainSwitchScheduler] Initialized (% version)").format(version).postln;
        };
        now = SystemClock.seconds.round(0.001);
        ("[ChainSwitchScheduler] Ready at %".format(now)).postln;
        ^this
    }
    ...
    switchIn { |seconds|
        ...
        ("[ChainSwitchScheduler] Scheduled switch in % seconds."
    ...
    switchOnBeat { |beatClock, beatsAhead = 1|
        ...
        ("[ChainSwitchScheduler] Scheduled switch on beat % (tempoClock)."
